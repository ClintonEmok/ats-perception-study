...
describe('/stkde-3d comparison integration contracts', () => {
  ...
  test('retains stable comparison, source, signed-field, and preset contracts', () => {
    ...
    expect(stageSource).toContain('slot="B"');
    ...
    expect(viewportSource).toContain('w-full');
    expect(viewportSource).toContain('rounded-2xl');
    ...
    expect(intensityLegendSource).toContain('STKDE INTENSITY');
    expect(intensityLegendSource).toContain('getStkdePaletteGradient');
    expect(differenceSource).toContain('data-difference-field="signed-kde"');
    expect(differenceSource).toContain('w-full');
    expect(differenceSource).toContain('min-h-[20rem]');
